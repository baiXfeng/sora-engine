...
int Game::run() {
    ...
    this->initVariable();
    this->_delegate->init();
    if (_delegate->screenFillMode() == this->FIT) {
        if (auto render_target = screen().children().back()->to<RenderTargetWidget>(); render_target) {
            render_target->setRenderTargetSize( sdl_display_size );
            render_target->setPosition( sdl_window_size.to<float>() * 0.5f );
            render_target->setAnchor(0.5f, 0.5f);
            int width = sdl_display_size.x * sdl_window_size.y / float(sdl_display_size.y);
            render_target->setSize(width += width % 2, sdl_window_size.y);
            render_target->enableClip(true);
        }
    }
...
int initSDL(Game& game) {
    ...
    if (game.delegate()->screenFillMode() == game.STRETCHING) {
